                ...
                <td class="td_2">
                    ...
                    @if(Session::get('success'))
                        ...
                        <div class="detail_box">
                            ...
                            <form id="webconfig" method="POST" enctype="multipart/form-data" action="/sys_edit_4/post">
                                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                    <tbody>
                                    <tr>
                                        <th>版權宣告</th>
                                        <td>
                                            ...
                                            <div>
                                                <textarea cols="80" rows="5" class="v_01"
                                                          name="copyright_ch">{{ $webconfig[0]->copyright_ch }}</textarea>
                                            </div>
                                            <h3>简体中文</h3>

                                            <div>
                                                <textarea cols="80" rows="5" class="v_01"
                                                          name="copyright_cn">{{ $webconfig[0]->copyright_cn }}</textarea>
                                            </div>
                                            <h3>English</h3>

                                            <div>
                                                <textarea cols="80" rows="5" class="v_01"
                                                          name="copyright_en">{{ $webconfig[0]->copyright_en }}</textarea>
                                            </div>
                                            <h3>日本語</h3>

                                            <div>
                                                <textarea cols="80" rows="5" class="v_01"
                                                          name="copyright_jp">{{ $webconfig[0]->copyright_jp }}</textarea>
                                            </div>
                                            <h3>한국어</h3>

                                            <div>
                                                <textarea cols="80" rows="5" class="v_01"
                                                          name="copyright_kr">{{ $webconfig[0]->copyright_kr }}</textarea>
                                            </div>
...
<script>
    function toSubmit(next) {
        if (next) {
            $("form#webconfig").attr("action", "/sys_edit_4/post/next");
        }
        else {
            $("form#webconfig").attr("action", "/sys_edit_4/post");
        }
