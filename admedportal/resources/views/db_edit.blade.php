...
<form id="db_edit" method="POST" action="/db_edit">
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <th>資料庫名稱(&#8226;)</th>
    <td><input class="v_01" type="text" name="database_name" value="{{ $querydatabase[0]->database_name}}"></td>
  </tr>
  <tr>
    <th>嵌入語法(&#8226;)</th>
    <td><textarea class="v_02" cols="80" rows="10" name="syntax">{{ $querydatabase[0]->syntax }}</textarea></td>
  </tr>
  <tr>
    <th>是否顯示</th>
    <td>

    @if( $querydatabase[0]->view == 1)    
      <label><input type="radio" name="view" value="1" checked>是</label>
      <label><input type="radio" name="view" value="0">否</label>
    @elseif( $querydatabase[0]->view == 0)
  ...
  <tr>
    <th>排序</th>
    <td><input class="v_00" type="text" name="rank_id" value="{{ $querydatabase[0]->rank_id }}">
    <div class="note_txt">數字愈大，順序愈前面。</div>
    </td>
  </tr>
  <tr>
    <th>備註</th>
    <td><textarea rows="5" name="note">{{ $querydatabase[0]->note }}</textarea></td>
  </tr>
  <tr>
    <th>&nbsp;</th>
    <td>
    <a class="btn_02" href="javascript:history.go(-1);">返回</a>
    <a class="btn_02" onClick="document.getElementById('db_edit').submit();">送出</a>
    </td>
  </tr>
</table>

    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <input type="hidden" name="id" value="{{ $querydatabase[0]->id }}">
