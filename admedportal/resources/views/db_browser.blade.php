...
<!-- 功能 區塊 Begin -->
<div class="func_box">
<a class="btn_02" href="{{ route('db.add') }}">新增</a>
</div>
<!-- 功能 區塊 End -->
  ...
  @if(count($querydatabase))
    ...
    <tr>
        ...
        @elseif( $querydatabase[$i]->view == 0)
            <td>否</td>
        @endif        
        <td>{{ $querydatabase[$i]->note }}</td>
        <td>{{ $querydatabase[$i]->created_at }}</td>
        <td>{{ $querydatabase[$i]->updated_at }}</td>
        <td>
            <a class="btn_03" href="{{ $url = route('db.browser.id.delete', ['id' => $querydatabase[$i]->id ] ) }}">刪除</a>
            <a class="btn_02" href="{{ $url = route('db.edit.id', ['id' => $querydatabase[$i]->id ]) }}">修改</a>
        </td>
    </tr>
